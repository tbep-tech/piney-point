...
load(url('hhttps://tbep-tech.github.io/piney-point/data/loboeco.RData'))
...
ecosel <- reactive({
  ...
  out <- loboeco %>% 
    filter(as.Date(Date) >= dtsel[1] & as.Date(DateTimeStamp) <= dtsel[2])
...
ecoplo <- reactive({
  ...
  out <- WtRegDO::plot(ecosel, by = 'days')
