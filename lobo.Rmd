...
tsplo <- reactive({
  ...
  p1 <- ggplot(datsel, aes_string(x = 'DateTimeStamp', y = varsel1)) + 
    ...
    annotate('text', x = evnts[1] + 2e4, y = ytxt, hjust = 0, label = 'Active discharge', color = 'orange') +
...
ecoplo <- reactive({
  ...
  out <- plot(ecosel, by = 'days') + 
    ...
    annotate('text', x = as.Date(evnts[1]) + 0.2, y = ytxt, hjust = 0, label = 'Active discharge', color = 'orange') +
    coord_cartesian(xlim = dtsel) + 
