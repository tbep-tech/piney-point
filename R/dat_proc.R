...
# salinity was not availabe at sites, so estimated from rswqdat
...
biostatspopulation <- biostats %>%
  ...
  dplyr::mutate(
    Salinity = 30, # this was the general salinity beginning of April
...
fl <- fls[which(fls$name == 'FLDEP_20211017'), 'id'] %>% pull(id)
flsht <- read_sheet(fl)
out1 <- flsht %>% 
  clean_names %>% 
  select(
...
# rswqdat <- rswqdat %>%
#   select(station, date, source, var, uni, val, qual) %>%
#   filter(!source %in% c('fldep', 'epchc'))
# rswqdat <- bind_rows(fldep1, epc1, rswqdat) %>%
