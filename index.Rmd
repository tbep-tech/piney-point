...
column(12,
  ...
  column(4,
    renderUI(
      selectInput(inputId = 'savsel', label = HTML('<b>Select SAV species:</b>'), choices = rstrnsavsel(), selected = rstrnsavsel(), multiple = T, selectize = T, width = '100%')
    )
  ),
  column(4,
    renderUI(
      selectInput(inputId = 'mcrsel', label = HTML('<b>Select macroalgae group:</b>'), choices = rstrnmcrsel(), selected = rstrnmcrsel(), multiple = T, selectize = T, width = '100%')
    )
  )
)
```
...
```{r}
output$rstrnplo1 <- renderGirafe(rstrnplo1())
girafeOutput('rstrnplo1', height = '100%', width = '100%')
```
