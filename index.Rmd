...
load('data/macropts.RData')
load('data/macrolns.RData')
...
mctrns <- sort(unique(macrodat$station))
...
sgmap <- sgmap %>% 
  ...
  addLabelOnlyMarkers(data = trnpts, ~0.99995 * lng, ~lat, label = ~station, labelOptions = labelOptions(
    noHide = T, 
    direction = 'auto', 
    textOnly = T,
    textsize = '15px',
...
# macroalgae base map
mcmap <- mapview(macropts, homebutton = F, legend = F) %>% 
  .@map %>% 
  clearMarkers()
for(i in unique(macrolns$station))
  mcmap <- mcmap %>% 
    addPolylines(
      data = macrolns[macrolns$station %in% i, ], 
      lng = ~lng, 
      lat = ~lat, 
      color = 'grey', 
      label = ~station
...
mcmap <- mcmap %>% 
  ...
  addLabelOnlyMarkers(data = macropts, ~0.99995 * lng, ~lat, label = ~station, labelOptions = labelOptions(
    noHide = T, 
    direction = 'auto',
    textOnly = T,
    textsize = '15px',
...
# macroalgae baseline plot
bsmctrnplo1 <- reactive({
  
  # input
  trnsel2 <- input$trnsel2
  
  p <- show_macrotransect(macrodat, station = trnsel2, plotly = T, base_size = 11) %>% 
    layout(
      title = list(text = NA), 
      yaxis = list(title = NA),
      legend = list(title = list(text = 'Weight (g)'))
    )
...
1) __CURRENT DATA__: Locations being sampled by our partners in response to changing conditions, with an [overview summary](https://shiny.tbep.org/piney-point/#section-water-quality-overview) of sampling events to date and [current results](https://shiny.tbep.org/piney-point/#section-water-quality-results).
1) __BASELINE DATA__: Long-term [water quality](https://shiny.tbep.org/piney-point/#section-water-quality) and [seagrass data](https://shiny.tbep.org/piney-point/#section-seagrasses) over a 25 year period.  These represent "baseline" conditions.
...
<!-- MACROALGAE {data-navmenu="BASELINE DATA"} -->
<!-- ======================================================================= -->
...
<!-- ```{r} -->
<!-- output$bsmcmap1 <- renderLeaflet(bsmcmap1()) -->
<!-- leafletOutput('bsmcmap1') -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- fillCol(flex = c(0.1, 0.4), -->
<!--   column(12, -->
<!--     selectInput(inputId = 'trnsel2', label = 'Select transect:', choices = mctrns, selected = mctrns[1]) -->
<!--   ), -->
<!--   # fillCol(flex = c(1), -->
<!--     renderPlotly(bsmctrnplo1()) -->
<!--   # ) -->
<!-- ) -->
<!-- ``` -->
