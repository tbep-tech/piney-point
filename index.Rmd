...
rsmap1 <- reactive({
  ...
  if(nrow(tomap) != 0 & rngemph){
    ...
    if(nrow(tomap1) == 0)
      m <- mapview(tomap2, cex = tomap2$cexs, label = labs2, legend = F, layer.name = F, col.regions = tomap2$cols, homebutton = F)
    ...
    if(nrow(tomap1) > 0 & nrow(tomap2) > 0)
      m <- mapview(tomap1, cex = tomap1$cexs, label = labs1, legend = F, layer.name = F, col.regions = tomap1$cols, homebutton = F, map.types = mptyps, lwd = 0) +
        mapview(tomap2, cex = tomap2$cexs, label = labs2, legend = F, layer.name = F, col.regions = tomap2$cols, homebutton = F)
