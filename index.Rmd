...
rsbntmap <- reactive({
  ...
  out <- mapview(tomap, homebutton = F, legend = F) %>% 
    ...
    addCircleMarkers(
        data = tomap[tomap$station %in% rsbnts[1], ],
        layerId = 'selpt',
        stroke = TRUE,
        color = 'black',
        fill = TRUE,
