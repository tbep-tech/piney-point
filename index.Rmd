...
# logs
rswqlog <- readLines('logs/rswqlog.txt')
rscntlog <- readLines('logs/rscntlog.txt')
rsphylog <- readLines('logs/rsphylog.txt')
rstrnlog <- readLines('logs/rstrnlog.txt')
rsbntlog <- readLines('logs/rsbntlog.txt')
...
# benthoc months
rsbntsmodt <- sort(unique(rsbntdat$modt))
...
# seagrass base map
bssgmap <- mapview(trnpts, homebutton = F, legend = F, map.types = mptyps) %>% 
  .@map %>% 
  clearMarkers()
...
rstrnlns <- rstrnlns %>% 
  filter(station %in% rstrns)
rstrnmap <- mapview(rstrnpts, homebutton = F, legend = F, map.types = mptyps) %>% 
  .@map %>% 
  clearMarkers()
...
# binding for benthic marker select
makeReactiveBinding('stasel2')

# benthic station selection map click
observeEvent(input$rsbntmap1_marker_click, {
  stasel2 <<- input$rsbntmap1_marker_click$id
})

# rapid response base benthic map
rsbntmap <- reactive({
  ...
  # select month to map
  tomap <- rsbntdat %>% 
    filter(modt %in% bntsel) %>% 
    select(station, col, TBBICat) %>% 
  ...
  out <- mapview(tomap, homebutton = F, legend = F, map.types = mptyps) %>% 
    .@map %>% 
    clearMarkers() %>% 
    addCircleMarkers(
      data = tomap,
    ...
    addCircleMarkers(
        data = tomap[tomap$station %in% stasel2, ],
        layerId = 'selpt',
        stroke = TRUE,
        color = 'black',
        fill = TRUE,
    ...
    addMarkers(lng = pineypoint[1], lat = pineypoint[2], label = 'Piney Point', options = markerOptions(interactive = FALSE)) %>% 
    addLegend("topright", title = 'Benthic condition', opacity = 1, labels = c('Healthy', 'Intermediate', 'Degraded'), colors = c('darkgreen', 'yellow', 'red')) %>% 
    setView(mapcenter[1], mapcenter[2], zoom = 11)
...
observeEvent(input$rsbntmap1_marker_click, {
  ...
  toadd <- rsbntdat %>% 
    ...
    filter(modt %in% bntsel) %>% 
    inner_join(rsbntpts, ., by = 'station') %>% 
    select(station, source_lng, TBBICat, col)
...
# rapid response benthic gauge
rsbntgge1 <- reactive({
  
  # input
  rsbntdat1 <- rsbntdat1()

  out <- gauge(round(rsbntdat1$TBBI, 1), min = 0, max = 100, label = '', gaugeSectors(
    danger = c(0, 73), warning = c(73, 87), success = c(87, 100),
...
rsbntplo1 <- reactive({
  ...
  cols <- c("#FFF5EB", "#FEE6CE", "#FDD0A2", "#FDAE6B", "#FD8D3C", "#F16913", 
            "#D94801", "#A63603", "#7F2704")
  
  p <- ggplot(toplo, aes(y = reorder(FAMILY, TotalAbundance), x = TotalAbundance)) + 
    geom_bar(stat = 'identity', aes(fill = TotalAbundance), colour = 'black', alpha = 0.6) + 
    scale_x_continuous(expand = c(0, 0), limits = c(0, max(rsbntdat$TotalAbundance))) +
    ...
    labs(
      x = 'Abundance (indviduals / m2)'
    )
  
  if(bntscl)
    p <- p + 
      scale_x_log10(expand = c(0, 0), limits = c(1, max(rsbntdat$TotalAbundance))) +
...
rsphymap <- reactive({
  ...
  out <- mapview(tomap, homebutton = F, legend = F, map.types = mptyps) %>% 
    ...
    addCircleMarkers(
      ...
      fillOpacity = 1,
      radius= 4,
      label = ~paste0('Station: ', station, ' (', source_lng, ')')
    ) %>% 
    addCircleMarkers(
      data = toadd,
      layerId = 'selpt',
...
rsphymap2 <- reactive({
  ...
  out <- mapview(rsphypts, homebutton = F, legend = F, map.types = mptyps) %>% 
    .@map %>%
    clearMarkers() %>% 
    clearControls() %>% 
    addCircleMarkers(
...
rsphyplo1 <- reactive({
  ...
  if(typ == 'Quantitative'){
    ...
    cols <- c('white', cols)
    unidts <- length(unique(toplo$date))
...
1) __CURRENT DATA__: Current [water quality](#water-quality-results), [algal surveys](#algal-surveys), [seagrass/macroalgae](#rapid-response-seagrassmacroalgae-surveys), [contaminant](#contaminant-results), and [sediment biology](#benthic-biology) results are presented.
...
### __STATION LOCATIONS - PINEY POINT AT MARKER__, last update `r rswqlog`
...
column(12,
  ...
  column(9, 
    renderUI({
      ...
      sliderInput('dyssel1', 'Select date range:', min = min(dys), max = max(dys), value = range(dys), animate = T, timeFormat="%m-%d-%Y", width = '100%')
...
__Nitrate/Nitrite (mg/L)__: Nitrate/nitrite concentration as a component of total nitrogen.  Excess nitrate/nitrite can stimulate algal production.  Concentrations are presented as the sum of both.
...
```{r, out.width = '100%'}
knitr::include_url('data/wqrefmap.html', height = '100%')
```
...
column(12, 
  column(9,
    sliderInput('dyssel3', 'Select date range:', min = min(rsphydys), max = max(rsphydys), value = range(rsphydys), animate = T, timeFormat="%m-%d-%Y", width = '100%')
  )   
)
```
...
BENTHIC BIOLOGY {data-navmenu="CURRENT DATA"}
=======================================================================

Column {data-width=400}
-----------------------------------------------------------------------

### __STATION LOCATIONS - PINEY POINT AT MARKER__, last update `r rsbntlog`
...
column(12, 
  ...
  column(4, 
    selectInput(inputId = 'bntsel', label = HTML('<b>Select sample date:</b>'), choices = rsbntsmodt)
  ), 
  column(4, 
       materialSwitch(inputId = 'bntscl', label = 'Show abundance in log-scale?', value = T, width = '100%')     
  )
)
```
...
```{r}
output$rsbntgge1 <- renderGauge(rsbntgge1())
output$rsbntplo1 <- renderPlot(rsbntplo1())
output$rsbnttxt1 <- renderUI(HTML(paste0('<b>Benthic condition</b>: ', rsbntdat1()$TBBICat)))

fillCol(flex = c(NA, NA, NA, 1),
  uiOutput('rsbnttxt1'),
  gaugeOutput('rsbntgge1', width = 400, height = 900), 
  HTML('<b>Family abundance</b>'), 
  plotOutput('rsbntplo1')
)
```

### __INFORMATION__

Initial sediment samples near Piney Point were collected 2021-04-07. These samples will be compared with samples collected in the fall to determine a potential impact of Piney Point on the benthic communities. 

For each sample at each station, benthic organisms living in the sediment were identified and enumerated.  This information is used to calculate the Tampa Bay Benthic Index (TBBI) to quantify the ecological health of the benthic community as affected by the the quality of the sediments they inhabit.  Samples are scored as Healthy, Intermediate, or Degraded based on continuous TBBI scores from 0 - 100 (negative scores are possible). Contaminants and other pollutants from the Piney Point effluent may negatively impact benthic communities and the TBBI will reflect these changes. 

The graphics on the __STATION RESULTS__ tab show the estimated TBBI score for the selected site (top) and a plot of the abundances of organisms found at the site (bottom). 

Additional information regarding the Tampa Bay Benthic Monitoring Program is available at <a href="https://tbep.org/tampa-bay-benthic-index/">https://tbep.org/tampa-bay-benthic-index/</a>.
