...
stacols <- colorRampPalette(RColorBrewer::brewer.pal(n = 7, name = 'Accent'))(length(unique(rsstatloc$source_lng)))
...
bswqtxt1 <- reactive({
  ...
  out <- HTML(paste('Normal range for March/April (mean +/- 1 sd):<b>', tmp$minv, '-', tmp$maxv, tmp$lbunis, '</b><br></br>'))
  ...
  if(rngemph)
    out <- reactable(totab,
      ...
      rowStyle = JS("function(rowInfo) {
        if (rowInfo.row['In normal range?'] == 'no') {
...
rsmap1 <- reactive({
  ...
  if(nrow(tomap) != 0 & rngemph){
    ...
    tomap1 <- tomap %>% filter(inrng == 'yes')
    tomap2 <- tomap %>% filter(inrng == 'no')
...
# rapid response phyto map selection
rsphymap1 <- reactive({
...
rsphytxt1 <- reactive({
  ...
  out <- HTML(paste0('<b>', typ, ' samples</b><br></br>'))
...
bswqtxt2 <- reactive({
  ...
  out <- HTML(paste(tmp$lbs, 'normal range for March/April (mean +/- 1 sd):<b>', tmp$minv, '-', tmp$maxv, '</b><br></br>'))
...
rsphyplo1 <- reactive({
  ...
  if(typ == 'Quantitative'){
    ...
    out <- ggplot(toplo, aes(y = species, x = val)) + 
      ...
      scale_fill_manual(values = RColorBrewer::brewer.pal(length(levels(toplo$valqual)), 'Reds')) +
      facet_wrap(~date) + 
...
[__JUMP TO LATEST DATA__](#water-quality-results), last update `r log`
...
1) __CURRENT DATA__: Locations being sampled by our partners in response to changing conditions, with an [overview summary](#water-quality-overview) of sampling events to date and [current results](#water-quality-results). Current  [seagrass/macroalgae](https://shiny.tbep.org/piney-point/#section-rapid-response-seagrassmacroalgae-surveys) data are also presented. [Contaminants](#contaminant-results), [sediment](#benthic-surveys) (biology and chemistry), and [algal samples](#algal-surveys) have also been collected and the data are being processed.
...
* USF Optical Oceanography Lab algal bloom satellite imagery: <https://optics.marine.usf.edu/cgi-bin/animation?productSensor=MPH.olcis3a&amp;animateFrom=03/29/2021&amp;animateTo=04/30/2021&amp;roi=CWFL>
...
1. You can choose to emphasize results that are out of the normal range of values for the March/April period in Tampa Bay.  Points within the normal range will no longer be outlined on the map, making those outside of range that are easier to view.  The same values that are out of range will be highlighted in the table.  The "normal range" is based on the long-term March/April average plus and minus 1 standard deviation of the mean for each parameter.  This range is displayed as text below the selection options. 
...
```{r}
output$rsphymap1 <- renderLeaflet(rsphymap1())
...
column(12, 
  column(4, 
    selectInput(inputId = 'stasel3', label = 'Select station:', choices = rsphys, selected = rsphys[1])
    ), 
  column(4,
    renderUI({
      ...
      selectInput('varsel2', 'Select water quality variable:', choices = tosel, selected = tosel[1])
...
This tab shows results of phytoplankton (algae) surveys provided from different sources.  The data are either qualitative (e.g., description of taxa observed at a site) or quantitative (counts of taxa observed at a site) depending on the data source.  Phytoplankton are collected at the same stations where water quality samples are collected, which allows a comparison of both to determine potential associations between water quality (e.g., nutrient concentrations) and changes in the algal community.  

Select the station to view from the menu on the top.  The station map on the left shows which stations provide qualitative or quantitative data.  If a qualitative station is selected, a tabular summary of species observed by date will be shown.  If a quantitative station is selected, a plot of algal cell counts (a measure of abundance) by date will be shown.  

For all stations, water quality data collected at the station will also be shown in a table below the algal results.  You can choose which water quality variable is shown. Observations outside of the normal range will be highlighted in the table. 
