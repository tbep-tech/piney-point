...
rsmap1 <- reactive({
  ...
  if(nrow(tomap) != 0 & !rngemph){
    ...
    m <- mapview(tomap, cex = tomap$cexs, label = labs, legend = F, layer.name = F, col.regions = tomap$cols, homebutton = F, map.types = mptyps, layerId = tomap$station, popup = NULL)
  ...
  if(nrow(tomap) != 0 & rngemph){
    ...
    if(nrow(tomap1) == 0)
      m <- mapview(tomap2, cex = tomap2$cexs, label = labs2, legend = F, layer.name = F, col.regions = tomap2$cols, homebutton = F, map.types = mptyps, layerId = tomap2$station, popup = NULL)
  
    if(nrow(tomap2) == 0)
      m <- mapview(tomap1, cexbs = tomap1$cexs, label = labs1, legend = F, layer.name = F, col.regions = tomap1$cols, homebutton = F, map.types = mptyps, lwd = 0, layerId = tomap1$station, popup = NULL)
    
    if(nrow(tomap1) > 0 & nrow(tomap2) > 0)
      m <- mapview(tomap1, cex = tomap1$cexs, label = labs1, legend = F, layer.name = F, col.regions = tomap1$cols, homebutton = F, map.types = mptyps, lwd = 0, layerId = tomap1$station, popup = NULL) +
        mapview(tomap2, cex = tomap2$cexs, label = labs2, legend = F, layer.name = F, col.regions = tomap2$cols, homebutton = F, map.types = mptyps, layerId = tomap2$station, popup = NULL)
...
1. You can choose to show all dates or filter by a particular date. If results are filtered by date, hit the "play" button below the date slider to view a time lapse of data. The data table will show only results for the selected day.  __You can also click on a station to view the complete time series for a parameter of interest.__
...
The "normal range" is based on the long-term March/April average plus and minus 1 standard deviation of the mean for each parameter.  The long-term values that are compared to the current results are from baseline data at the permanent (reference) station nearest to the current water quality station. Many, but not all, of the current sampling stations are new locations selected to best characterize potential changes in water quality near Piney Point. See the [REFERENCE STATIONS](https://shiny.tbep.org/piney-point/#section-reference-stations) map to view the current water quality stations relative to the reference stations used to establish the normal range.  The station used as reference for the normal range is shown in the table, together with the estimated normal range. The normal ranges change by location.  If a parameter measured at the current station is unavailable for comparison at the nearest reference station, the normal range is used from the next closest reference station.  
...
```{r}
column(12, 
  column(6, HTML('<b>Select a station by clicking a marker on the map</b>')), 
  column(6,
    renderUI({
...
fillCol(flex = c(0.1, 0.9),
  fillRow(
    ...
    renderUI(
      selectInput(inputId = 'mcrsel', label = HTML('<b>Select macroalgae group:</b>'), choices = rstrnmcrsel(), selected = rstrnmcrsel(), multiple = T, selectize = T)
    )
  ),
...
Macroalgae taxa are categorized by major groups that include *Red*, *Green*, *Brown*, and *Cyanobacteria*.  Macroalgae may respond more rapidly to nutrient inputs than SAV and may serve as early warning indicators for biological response to changing water quality.
